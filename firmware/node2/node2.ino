...
// -----------------------------------------------------------------------------
// ESP32-S3 PHONE LOCALIZER - PASSIVE ANCHOR NODE 2
// Arduino-ESP32 3.x, Board: "ESP32S3 Dev Module"
// -----------------------------------------------------------------------------

static const uint8_t ANCHOR_ID = 2;
static const uint8_t WIFI_CHANNEL = 6;
static const uint8_t MASTER_AP_MAC[6] = {0x02, 0x4C, 0x4F, 0x43, 0x00, 0x01};
static const uint8_t THIS_NODE_STA_MAC[6] = {0x02, 0x4C, 0x4F, 0x43, 0x02, 0x01};

// These keys must exactly match the master and Node 1.
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("\nESP32-S3 Localizer Anchor Node 2 starting...");
  ...
  if (esp_wifi_set_mac(WIFI_IF_STA, THIS_NODE_STA_MAC) != ESP_OK) {
    Serial.println("ERROR: Could not set Node 2 MAC.");
  }
